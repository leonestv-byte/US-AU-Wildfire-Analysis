...
#######################################
##### AU - No Fires Full Dataset  #####
#######################################



au_data <- read.csv("MSISS/wildfires/main/data/au/au_fires_full_dataset.csv")



# 1) Load all raw fields

fire_data <- au_data[, c("latitude", "longitude", "brightness", "daynight", "acq_date", "acq_time", "daynight", "type", "state", "city", "population", "datetime")]
...
########################################
##### AU - Sample from the Region  #####
########################################

aus <- ne_countries(country = "Australia", returnclass = "sf")
n_nonfire <- nrow(au_data) * 2 
...
###################################################
##### AU - Remove Samples Too Close to Fires  #####
###################################################
...
write.csv(
  nonfire,
  "MSISS/wildfires/main/data/au/au_no_fires_full_dataset.csv",
  row.names = FALSE
)
